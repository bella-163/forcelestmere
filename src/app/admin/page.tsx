    ...
    return (
      <div className="flex min-h-screen items-center justify-center px-5">
        <div className="w-full max-w-sm">
          <p className="mb-2 text-sm font-black tracking-[0.22em] text-cele-teal">ADMIN</p>
          ...
          <form onSubmit={login} className="space-y-4">
            <input
              ...
              className="w-full rounded-xl border border-white/15 bg-white/5 px-4 py-3 text-white placeholder-white/30 outline-none focus:border-cele-teal/60 focus:ring-1 focus:ring-cele-teal/30"
            />
            {error && <p className="text-sm text-red-400">{error}</p>}
            <button
              type="submit"
              disabled={logging}
              className="w-full rounded-xl bg-cele-teal px-4 py-3 font-black text-white transition hover:bg-cele-teal/85 disabled:opacity-50"
            >
              {logging ? "登入中..." : "登入"}
            </button>
          </form>
  ...
  return (
    <div className="mx-auto max-w-4xl px-5 py-16">
      <div className="mb-10 flex items-center justify-between">
        <div>
          <p className="text-sm font-black tracking-[0.22em] text-cele-teal">ADMIN PANEL</p>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {SECTIONS.map((s) => (
          <Link
            key={s.href}
            href={s.href}
            className="group rounded-2xl border border-white/10 bg-white/[0.04] p-6 transition hover:-translate-y-1 hover:border-cele-teal/40"
          >
            <h2 className="text-xl font-black text-white group-hover:text-cele-teal">{s.label}</h2>
            <p className="mt-2 text-sm text-white/55">{s.desc}</p>
            <p className="mt-4 text-sm font-bold text-white/30 group-hover:text-cele-teal/70">編輯 →</p>
          </Link>
        ))}
      </div>
    </div>
